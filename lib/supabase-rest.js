 ...
 *
 * 환경변수 (우선순위 — RLS 우회를 위해 service_role/secret 키 우선 선택):
 *   SUPABASE_URL                = https://xxxxxxxxxxxx.supabase.co
 *   1) SUPABASE_SERVICE_ROLE_KEY (legacy 명칭, RLS 우회)        ← 권장
 *   2) SUPABASE_SECRET_KEY       (신 명칭, RLS 우회)            ← 권장
 *   3) SUPABASE_PUBLISHABLE_KEY  (anon 등급, RLS 정책 적용)     ← fallback
 *
 * 본 래퍼는 서버사이드 API route 에서만 사용하므로 service_role 키가 최적.
 * publishable/anon 키만 있으면 RLS 정책에 따라 INSERT 차단(42501) 발생.
 *
