...
// 카테고리별 기본 aspect 배열 — 페이지 의도(증상/치료/원인/예방)를 풍부하게 전달
const ASPECT_DEFAULT = {
  Symptom:   ['Symptom', 'Causes', 'Treatment', 'Prevention'],
  Treatment: ['Treatment', 'Indication', 'SideEffect'],
}

export function medicalWebPageSchema({ title, description, url, updated, aspect, conditionName }) {
  const aspects = Array.isArray(aspect)
    ? aspect
    : (ASPECT_DEFAULT[aspect] || ASPECT_DEFAULT.Symptom)
  return {
    '@context': 'https://schema.org',
    '@type': 'MedicalWebPage',
    name: title,
    ...
    // 정식 의료 청중 속성 (Patient 대상 일반 정보)
    medicalAudience: { '@type': 'MedicalAudience', audienceType: 'Patient' },
    // 하위호환 — 일부 크롤러는 일반 audience 도 함께 읽음
